...
static __always_inline void rc_bittree_reverse(struct rc_dec* rc, uint16_t* probs, uint32_t* dest,
                                               uint32_t limit) {
    uint32_t symbol = 1;
    uint32_t i = 0;
...
enum xz_ret xz_dec_microlzma_run(struct xz_dec_microlzma* s_ptr, struct xz_buf* b) {
    ...
    if (s->lzma2.sequence != SEQ_LZMA_RUN) {
         ...
         * ever need.
         */
        if (s->lzma2.compressed < RC_INIT_BYTES || s->lzma2.compressed > (3U << 30))
            return XZ_DATA_ERROR;
    ...
    while (true) {
        ...
        if (b->in_pos == b->in_size && s->temp.size < s->lzma2.compressed)
            return XZ_OK;
    }
}
...
void xz_dec_microlzma_reset(struct xz_dec_microlzma* s, uint32_t comp_size, uint32_t uncomp_size,
